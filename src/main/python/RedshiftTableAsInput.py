...
class RedshiftTableAsInput(InputSourceAbstract):
    ...
    def readInputDataCreateSparkDF(self, spark, config,params):
        ...
        #               str(params['end_date'])))
        user, passwd, host, port = connection.getRedshiftConnectionParams()
        db = config['input_data_params']['db']
        if config['input_data_params']['custom_query_to_run'].upper()=='NO':
            ...
            inputDf = spark.read.format("io.github.spark_redshift_community.spark.redshift") \
                .option("url", "jdbc:redshift://{0}:{1}/{2}?user={3}&password={4}".format(host, port, db,
                                                                                          user, passwd)) \
                .option("query", select_cond) \
                .option("tempdir", "s3://-dev/Milind/tmp/") \
                .option("aws_iam_role", "arn:aws:iam:::role/Role_for_redshift_data_copy_unload") \
                .load()
